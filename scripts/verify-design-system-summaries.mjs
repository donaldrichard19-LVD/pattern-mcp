 ...
 *
 * Free, deterministic check of register_design_system's `summarize` (on by default)
...
console.log("\n=== 1. Default: ON when a key is set; three ways to keep it local ===");
{
  anthropicRequests.length = 0;
  const dflt = parse(await register(client, "d1"));
  check("no flag + key set -> summaries are generated by default", dflt.body?.summaries?.generated === 3 && anthropicRequests.length === 3);
  check("response carries a plain notice that source text was sent", /sent to api\.anthropic\.com/.test(dflt.body?.summaries?.notice ?? "") && /summarize: false/.test(dflt.body.summaries.notice));
  anthropicRequests.length = 0;
  const off = parse(await register(client, "d2", { summarize: false }));
  check("summarize: false -> no Anthropic call, no summaries", anthropicRequests.length === 0 && off.body.registration.candidates.every((c) => !c.summary) && off.body.summaries === undefined);
  const envOff = await connect({ PATTERN_NO_SUMMARIES: "1" });
  const viaEnv = parse(await register(envOff, "d3"));
  check("PATTERN_NO_SUMMARIES=1 -> no Anthropic call, no summaries", anthropicRequests.length === 0 && viaEnv.body.registration.candidates.every((c) => !c.summary));
  const envButExplicit = parse(await register(envOff, "d4", { summarize: true }));
  check("explicit summarize: true still overrides the env switch", envButExplicit.body?.summaries?.generated === 3);
  await envOff.close();
  const noKey = await connect({ ANTHROPIC_API_KEY: "" });
  anthropicRequests.length = 0;
  const skipped = parse(await register(noKey, "d5"));
  check("no key + default -> registration succeeds, reports it skipped, sends nothing", skipped.body?.status === "registered" && /not set/.test(skipped.body.summaries?.skipped ?? "") && anthropicRequests.length === 0);
  const manifest = parse(await noKey.callTool({ name: "register_design_system", arguments: { project_id: "d6", manifest_path: "manifest.json" } }));
  check("manifest + default -> fine, no summaries block (nothing to read)", manifest.body?.status === "registered" && manifest.body.summaries === undefined);
  await noKey.close();
}

console.log("\n=== 2. summarize: true (explicit) generates one summary per file ===");
{
  anthropicRequests.length = 0;
...
console.log("\n=== 3. Re-registering unchanged files reuses cached summaries (free) ===");
{
  ...
  const noFlag = parse(await register(client, "s1", { summarize: false }));
  check("even with summarize: false, unchanged files keep their summary", noFlag.body.registration.candidates.every((c) => !!c.summary));
...
console.log("\n=== 6. Refusals leave things untouched ===");
{
  ...
  check("explicit summarize: true without ANTHROPIC_API_KEY is refused", refused.isError === true);
